...
Future<void> watchStream(
  ...
  await Process.run(player.mpvExe, [
    ...
    '--ytdl-format=${player.quality.isNotEmpty ? player.quality : 'bestvideo+bestaudio'}',
    '--volume=${player.volume}',
    '--title=Zyron ${streamer.displayName}'.replaceAll(' ', '_'),
    '--autofit=50%x50%',
    '--geometry=50%x50%',
    '--sub-visibility=no',
    '--mute=no',
...
Future<void> watchVideo({
  ...
  await Process.run(player.mpvExe, [
    ...
    '--ytdl-format=${player.quality.isNotEmpty ? player.quality : 'bestvideo+bestaudio'}',
    '--volume=${player.volume}',
    '--title=Zyro $channelName | ${video.title}'.replaceAll(' ', '_'),
    '--autofit=50%x50%',
    '--geometry=50%x50%',
    '--mute=no',
    // '--force-window',
